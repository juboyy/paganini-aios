...
import click
from packs.finance.integrations.onboard import onboard_cli
...
def init(provider, model, api_key, corpus, rt, pack_id):
    ...
    from core.config.engine import save_config
    ...
    if pack_id:
        from core.packs.pack import PackManager
...
def query(question, no_llm, top_k, verbose):
    ...
    from core.metaclaw.engine import MetaClawProxy
...
def _run_llm(llm_fn, agent, context: str, question: str, pipeline, metaclaw, start: float):
    ...
    if llm_fn:
        ...
        system_prompt = (
            "Você é um especialista em fundos de investimento em direitos creditórios e regulamentação CVM.\n"
            f"{agent_context}\n"
            "Regras:\n"
...
def status():
    ...
    from core.rag.pipeline import RAGPipeline
    from core.moltis.llm import MoltisAdapter
    from core.metaclaw.engine import MetaClawProxy
...
def eval_cmd(eval_set, with_llm):
    ...
    from core.rag.pipeline import RAGPipeline
    from core.rag.eval import run_eval, print_report
    from core.moltis.llm import get_llm_fn
...
def up():
    ...
    dash_proc = subprocess.Popen(
        [venv_python, "core/channels/serve.py"],
        cwd=project_root,
    ...
    if tg_token:
        ...
        tg_proc = subprocess.Popen(
            [venv_python, "-c",
             f"from core.channels.telegram import run_bot; run_bot(token='{tg_token}')"],
            cwd=project_root,
...
def report_list():
    ...
    from packs.finance.integrations.reports import ReportGenerator
    from rich.table import Table
    config = _load_config()
...
def autoresearch(iterations, eval_set, verbose):
    ...
    try:
        from core.rag.autoresearch.runner import run_autoresearch
    except ImportError:
...
def serve(host, port):
    ...
    try:
        import uvicorn
        from core.channels.api import create_app
...
def modules_list():
    ...
    from core.packs.modules import list_modules
...
def modules_info(name):
    ...
    from core.packs.modules import load_module, get_module_agents, get_module_guardrails
...
def telegram_cmd(token):
    ...
    from core.channels.telegram import run_bot
    run_bot(token=token)
...
def shell_cmd():
    ...
    from core.channels.tui import run_shell
    run_shell()
