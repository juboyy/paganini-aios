...
def _get_engine():
    ...
    if _engine is None:
        try:
            from core.config.engine import load_config, create_engine
            config = load_config()
...
def cmd_market(**_):
    ...
    if not MARKET_SNAPSHOT.exists():
        ...
        try:
            from core.runtime.handlers import market_data_sync
...
def cmd_agents(**_):
    ...
    try:
        from core.config.engine import load_config, AgentRegistry
        config = load_config()
...
def cmd_query(text: str = "", **_):
    ...
    try:
        from core.config.engine import load_config, get_llm_fn
        config = load_config()
...
def cmd_daemons(**_):
    """Show daemon status."""
    try:
        from core.runtime.daemons import DaemonScheduler
        from core.config.engine import load_config
        config = load_config()
