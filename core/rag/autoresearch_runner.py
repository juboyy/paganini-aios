...
RAG_DIR = Path(__file__).resolve().parents[1]    # core/rag/
AR_DIR = Path(__file__).resolve().parent         # core/rag/autoresearch/
...
def call_llm(messages: list[dict], verbose: bool = False) -> str:
    ...
    try:
        from core.moltis.llm import get_llm_fn  # type: ignore
        llm = get_llm_fn()
